...
def buy_and_hold_benchmark(stock_name, env):
    df = pd.read_csv('./data/{}.csv'.format(stock_name)).iloc[977:, :]
...
def plot_all(stock_name, daily_portfolio_value, env):
    ...
    dates, buy_and_hold_portfolio_values, buy_and_hold_return = buy_and_hold_benchmark(stock_name, env)
